...
import uuid
# COCO API
from pycocotools.coco_text import COCO
...
def _filter_crowd_proposals(roidb, crowd_thresh):
    ...
    training.
    """
    print 'dont use this function! _filter_crowd_proposals'
    raw_input()
...
class coco_text(imdb):
    def __init__(self, image_set, year):
        ...
        self._data_path = osp.join(cfg.DATA_DIR, 'coco_text')
        # load COCO API, classes, class <-> id mappings
        print self._get_ann_file(), '*'*90
        self._COCO = COCO(self._get_ann_file())
        # cats = self._COCO.loadCats(self._COCO.getCatIds())
        # self._classes = tuple(['__background__'] + [c['name'] for c in cats])
        self._classes = tuple(['__background__', 'text'])
    ...
    def _load_image_set_index(self):
        ...
        if self._image_set == 'train':
            image_ids = self._COCO.train
        if self._image_set == 'val':
            image_ids = self._COCO.val
        if self._image_set == 'test':
            image_ids = self._COCO.test
