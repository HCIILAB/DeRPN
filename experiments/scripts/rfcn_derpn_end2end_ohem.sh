...
time ./tools/train_net.py --gpu ${GPU_ID} \
  --solver models/DeRPN/${PT_DIR}/${NET}/rfcn_end2end/solver_ohem.prototxt \
  --weights data/imagenet_models/${NET}-model.caffemodel \
  --imdb ${TRAIN_IMDB} \
  --iters ${ITERS} \
  --cfg experiments/cfgs/rfcn_derpn_end2end_ohem.yml \
  ${EXTRA_ARGS}
...
time ./tools/test_net.py --gpu ${GPU_ID} \
  --def models/DeRPN/${PT_DIR}/${NET}/rfcn_end2end/test_agnostic.prototxt \
  --net ${NET_FINAL} \
  --imdb ${TEST_IMDB} \
  --cfg experiments/cfgs/rfcn_derpn_end2end_ohem.yml \
  ${EXTRA_ARGS}
